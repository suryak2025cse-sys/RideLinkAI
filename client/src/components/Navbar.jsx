...
import { Shield, Car, User, Award, Wallet, AlertTriangle, MessageSquare, Leaf, LayoutDashboard, LogOut, ChevronDown, Bell } from 'lucide-react';
...
export default function Navbar() {
  ...
  const handleRoleSwitch = (newRole) => {
    ...
    dispatch(setCredentials({
      ...
      token: localStorage.getItem('token') || 'mock_jwt_token_2026'
    }));
            ...
            {isUserMenuOpen && (
              <div className="absolute right-0 mt-2 w-56 bg-white border border-slate-200 rounded-2xl shadow-xl py-2 z-50">
                <div className="px-4 py-2 border-b border-slate-100">
                  ...
                  <p className="text-xs text-slate-500">{user?.email || 'Sign in to access features'}</p>
                </div>
